...
class DappliRecipe {
  ...
  factory DappliRecipe.fromMap(Map<String, dynamic> data) {
    return DappliRecipe(
      favoriteRecipeIds: data['Idrecipe'] as String? ?? 'N/A',
...
class FavoriteProvider extends ChangeNotifier {
  ...
  List<String> get recipeFavoriteId =>
      _favoriteRecipes.map((r) => r.favoriteRecipeIds).toList();
  List<String> get recipeNames =>
      _favoriteRecipes.map((r) => r.recipeName).toList();
  ...
  Future<void> _storeRecipeFavoriteInFireBase(String recipeId, bool isAdding,
    ...
    try {
      ...
      final favoriteRef =
          userDocRef.collection('recipe_favorites').doc(recipeId);

      if (isAdding) {
        if (recipe != null) {
          await favoriteRef.set(recipe.toMap());
          if (kDebugMode) {
            print(
                "Added $recipeId to Firestore recipe favorites for ${currentUser.email}");
          }
        }
      } else {
        ...
        if (kDebugMode) {
          print(
              "Deleted $recipeId from Firestore recipe favorites for ${currentUser.email}");
        }
      }
    } catch (e) {
      if (kDebugMode) {
        print("Error in updating firestore: $e");
      }
    }
  }
    ...
    try {
      ...
      if (userDocQuery.docs.isNotEmpty) {
        ...
        final favoritesSnapshot =
            await userDocRef.collection('recipe_favorites').get();
        if (kDebugMode) {
          print(
              "Loaded ${favoritesSnapshot.docs.length} favorite recipes from sub-collection.");
        }
        for (final doc in favoritesSnapshot.docs) {
          try {
            final data = doc.data();
            _favoriteRecipes.add(DappliRecipe(
              favoriteRecipeIds: data['Idrecipe'] as String? ?? 'N/A',
              recipeName: data['name'] as String? ?? 'N/A',
              recipeImage: data['image'] as String? ?? 'N/A',
              recipeCalories: (data['calories'] as num?)?.toInt() ?? 0,
  ...
  Future<void> toggleRecipeFavorite(String recipeId, String name, String image,
    ...
    if (existingIndex != -1) {
      ...
      await _storeRecipeFavoriteInFireBase(recipeId, false);
    } else {
      final newRecipe = DappliRecipe(
      ...
      await _storeRecipeFavoriteInFireBase(recipeId, true, recipe: newRecipe);
    }
    notifyListeners();
  }

  Future<void> removeRecipeFavorite(String recipeId) async {
    final existingIndex = _favoriteRecipes
        .indexWhere((recipe) => recipe.favoriteRecipeIds == recipeId);
    ...
    if (existingIndex != -1) {
      ...
      notifyListeners();
      if (kDebugMode) {
        print("Removed favorite recipe $recipeId due to main recipe deletion.");
      }
    }
  }

  bool isRecipeExist(String recipeId) {
    return _favoriteRecipes
        .any((recipe) => recipe.favoriteRecipeIds == recipeId);
  }
}
//OLD CODE BELOW
